		...
		, m_scaling_factor(scaling_factor)
{
	...
	//set bounds
	bounds[0] = position.x - radius;
	bounds[1] = position.x + radius;
	bounds[2] = position.y - radius;
	bounds[3] = position.y + radius;
	bounds[4] = position.z - radius;
	bounds[5] = position.z + radius;
}
...
*/
	//update bounds, radius is always > 0
	bounds[0] = new_position.x - m_radius;
	bounds[1] = new_position.x + m_radius;
	bounds[2] = new_position.y - m_radius;
	bounds[3] = new_position.y + m_radius;
	bounds[4] = new_position.z - m_radius;
	bounds[5] = new_position.z + m_radius;
	assert(invariant());
}

double* Entity::getBounds() {
	double* temp = bounds;
	return temp;
}
...
//if collision
//use the velocity of each asteroid as their direction
//find average velocity as va = (m1 * v1 + m2 * v2) / (m1 + m2);
//find new velocity of each asteroid 
